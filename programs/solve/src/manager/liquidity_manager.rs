use super::{
    ...
    solve_manager::{next_solve_liquidity, next_solve_reward_infos},
...
pub fn calculate_modify_liquidity<'info>(
    ...
    let tick_lower = tick_array_lower.get_tick(position.tick_lower_index, solve.tick_spacing)?;

    let tick_upper = tick_array_upper.get_tick(position.tick_upper_index, solve.tick_spacing)?;
...
pub fn calculate_fee_and_reward_growths<'info>(
    ...
    tick_array_upper: &dyn TickArrayType,
    timestamp: u64,
) -> Result<(PositionUpdate, [SolveRewardInfo; NUM_REWARDS])> {
    let tick_lower = tick_array_lower.get_tick(position.tick_lower_index, solve.tick_spacing)?;
...
// Calculates the state changes after modifying liquidity of a solve position.
...
fn _calculate_modify_liquidity(
    ...
    let next_global_liquidity = next_solve_liquidity(
        solve,
        position.tick_upper_index,
        position.tick_lower_index,
        liquidity_delta,
    ...
    Ok(ModifyLiquidityUpdate {
        solve_liquidity: next_global_liquidity,
...
pub fn sync_modify_liquidity_values<'info>(
    ...
    solve.update_rewards_and_liquidity(
        modify_liquidity_update.reward_infos,
        modify_liquidity_update.solve_liquidity,
