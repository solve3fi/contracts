...
pub fn initialize_token_metadata_extension<'info>(
    ...
    // update authority: POSITION_NFT_UPDATE_AUTH
    invoke_signed(
...
pub fn build_position_token_metadata<'info>(
    ...
    // POSITION_2022_METADATA_NAME_PREFIX + " xxxx...yyyy"
    ...
    // POSITION_2022_METADATA_URI_BASE + "/" + pool address + "/" + position address
    // Must be less than 128 bytes
    let uri = format!(
        "{}/{}/{}",
        POSITION_2022_METADATA_URI_BASE,
        solve.key(),
        position.key(),
    );

    (name, POSITION_2022_METADATA_SYMBOL.to_string(), uri)
}
...
// Initializes a vault token account for a Solve.
