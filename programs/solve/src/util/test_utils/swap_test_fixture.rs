...
use crate::state::{
    solve_builder::SolveBuilder, tick_array::*, tick_builder::TickBuilder, FixedTickArray, Solve,
};
use crate::state::{
    AdaptiveFeeInfo, SolveRewardInfo, Tick, TickUpdate, MAX_TICK_INDEX, MIN_TICK_INDEX, NUM_REWARDS,
};
...
pub fn assert_swap(swap_update: &PostSwapUpdate, expect: &SwapTestExpectation) {
    ...
    assert_eq!(
        SolveRewardInfo::to_reward_growths(&swap_update.next_reward_infos),
