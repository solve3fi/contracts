...
pub fn load_tick_array<'a>(
    ...
    if tick_array.solve() != *solve {
        return Err(OrcaError::DifferentSolveTickArrayAccount.into());
    }
...
impl<'a> TickArraysMut<'a> {
    pub fn load(
        ...
        let lower_tick_array = load_tick_array_mut(lower_tick_array_info, solve)?;
        let upper_tick_array = if lower_tick_array_info.key() == upper_tick_array_info.key() {
            None
        } else {
            Some(load_tick_array_mut(upper_tick_array_info, solve)?)
        };
        Ok(Self {
