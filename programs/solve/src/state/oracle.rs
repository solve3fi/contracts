...
impl<'info> OracleAccessor<'info> {
    pub fn new(
        ...
        let oracle_account_initialized =
            Self::is_oracle_account_initialized(&oracle_account_info, solve.key())?;
        Ok(Self {
            oracle_account_info,
    ...
    fn is_oracle_account_initialized(
        ...
        if oracle_ref.solve != solve {
            // Just for safety: Oracle address is derived from Solve address, so this should not happen.
            unreachable!();
        }
